...
public class Main {
    ...
    private static void scanDirectory(File srcDir, File destDir) throws IOException {
        ...
        // 遍歷所有檔案和目錄
        for (File file : files) {
            // 如果是 edfile 資料夾，複製它裡面的所有檔案
            if (file.isDirectory() && file.getName().equals("edfile")) {
                copyFilesInFolder(file, new File(destDir.getAbsolutePath() + File.separator + file.getName()));
            }
            // 如果是子目錄，遞迴進入子目錄
            else if (file.isDirectory()) {
                //System.out.println("1destDir: " + destDir);
                // 如果目標目錄不存在，建立目錄

                scanDirectory(file, new File(destDir.getAbsolutePath() + File.separator + file.getName()));
            }
        }
    }
    // 取得 edfile 資料夾下所有檔案的 File 物件
    private static void copyFilesInFolder(File srcFolder, File destDir) throws IOException {
        // 取得目錄下所有檔案的 File 物件
        ...
        for (File file : files) {
            ...
            if (file.isFile()) {
                ...
                if (destFile.exists()) {
                    ...
                    //System.out.println("2destDir: " + destDir);
                    //將archive bit設為0
                    attrs.setArchive(false);
                    copyFile(file, destFile);
                    //比對兩個檔案是否一樣
            ...
            // 如果是子目錄，遞迴進入子目錄
                ...
                //System.out.println("3file: " + file.getName());
                //System.out.println("3destDir: " + destDir);
                copyFilesInFolder(file, new File(destDir.getAbsolutePath() + File.separator + file.getName()));
            }
        }
    }
