...
public class Main {
    ...
    public static void main(String [] argv) throws IOException  {
        ...
        String table = SQLScanner.next();  // 包含_ATT結尾
        ...
        //對input的table name做正規化
        String HaveATTtable = replaceAtt(table) + "_ATT";
        String NotHaveATTtable = replaceAtt(table);
        ...
        try (Connection con = DriverManager.getConnection(connectionUrl);) {
            ...
            //沒有_ATT結尾的
            if (!table.endsWith("_ATT")){
                String NOTATTtable = "SELECT * FROM " + owner + "." + NotHaveATTtable;
                Statement stmt1 = con.createStatement();
                ResultSet rs1 = stmt1.executeQuery(NOTATTtable);

                String Order = "SELECT * FROM " + owner + "." + NotHaveATTtable + " ORDER BY CONFIRM_DT ASC";
                ...
                ResultSet rs2 = stmt2.executeQuery(Order);

                //Initial config file
                File file = new File("C:\\Alg2Config.txt");
                FileWriter fw = new FileWriter("C:\\Alg2Config.txt", true);
                ...
                while (rs1.next()) {
                    ...
                    String CONFIRM_DT = rs1.getString("CONFIRM_DT");
                    ...
                    //找出要同步的檔案的數量
                    if(CONFIRM.equals("Y") && CONFIRM_DT.length() == 7 && Integer.valueOf(CONFIRM_DT) <= Integer.valueOf(Time)) {
                        TotalFileNum++;
                    }
                    else {
                        try (FileWriter writer = new FileWriter("C:\\Alg2Log.txt", true)) {
                            writer.write("over time of CRMYY: " + CRMYY + ", CRMID: " + CRMID + ", CRMNO: " + CRMNO + ", CHKNO: " + CHKNO + ", CONFIRM_DT: " + CONFIRM_DT + ", Time: " + Integer.valueOf(Time) + "\n");
                            writer.flush();
                ...
                while(total_count <= TotalFileNum ){
                    ...
                    rs2.absolute(dataIndex);
                    // 遍歷ResultSet對象，將第dataIndex個到dataIndex+1000個id存儲到List中
                    int timer = 0;
                    while (rs2.next() && timer < 1000) {
                        String CRMYY = rs2.getString("CRMYY");
                        String CRMID = rs2.getString("CRMID");
                        String CRMNO = rs2.getString("CRMNO");
                        String CHKNO = rs2.getString("CHKNO");
                    ...
                    for (int i = 0; i < idList.size(); i++) {
                        sb.append("SELECT * FROM " + owner + "." + NotHaveATTtable +  " WHERE ");
                        sb.append("(");
                        sb.append("CRMYY=");
                    ...
                    int ErrorCount = 0;
                    while(rs3.next()) {
                        String FILENM = rs3.getString("FILENM");
                        PublicCHKNO = rs3.getString("CHKNO");
                        PublicCRMYY = rs3.getString("CRMYY");
                        PublicCRMID = rs3.getString("CRMID");
                        PublicCRMNO = rs3.getString("CRMNO");
                        PublicFILENM = FILENM;
            ...
            //有_ATT結尾的
                ...
                //Select
                String ATT = "SELECT * FROM " + owner + "." + NotHaveATTtable + "_ATT";
                Statement stmt1 = con.createStatement();
                ResultSet rs1 = stmt1.executeQuery(ATT);

                String NOTATT = "SELECT * FROM " + owner + "." + NotHaveATTtable + " WHERE CRMYY=? AND CRMID=? AND CRMNO=? AND CHKNO=?";
                PreparedStatement stmt2 = con.prepareStatement(NOTATT);

                String Order = "SELECT CRMYY, CRMID, CRMNO, CHKNO FROM " + owner + "." + NotHaveATTtable + " ORDER BY CONFIRM_DT ASC";
                //創建statement要加上這些參數，才能透過過absolute去定位
                ...
                while (rs1.next()) {
                    ...
                    //去SCRIPT資料庫找到符合的CONFIRM_DT
                    while (rs2.next()) {
                        String CONFIRM_DT = rs2.getString("CONFIRM_DT");
                        ...
                        if (rs2.getString("CRMYY").equals(CRMYY) && rs2.getString("CRMID").equals(CRMID) && rs2.getString("CRMNO").equals(CRMNO) && rs2.getString("CHKNO").equals(CHKNO)) {
                            if (CONFIRM_DT.length() == 7 && Integer.valueOf(CONFIRM_DT) <= Integer.valueOf(Time)) {
                                TotalFileNum++;
                            }
                            else {
                                try (FileWriter writer = new FileWriter("C:\\Alg2Log.txt", true)) {
                                    writer.write("over time of CRMYY: " + CRMYY2 + ", CRMID: " + CRMID2 + ", CRMNO: " + CRMNO2 + ", CHKNO: " + CHKNO2 + ", CONFIRM_DT: " + CONFIRM_DT + ", Time: " + Integer.valueOf(Time) + "\n");
                                    writer.flush();
                                } catch (IOException ex) {
                                    ex.printStackTrace();
                                }
                ...
                while(total_count <= TotalFileNum ){
                    ...
                    // 將每個Key拼接成SQL查詢語句
                    for (int i = 0; i < idList.size(); i++) {
                        sb.append("SELECT * FROM " + owner + "." + NotHaveATTtable + "_ATT" +  " WHERE ");
                        sb.append("(");
                        sb.append("CRMYY=");
                        sb.append(idList.get(i).split(",")[0]);
                        sb.append(" AND CRMID=");
    ...
    public static boolean  ChangeFullPath(String SourceDrive, String DestDrive, String Prefix, String schema, StringBuilder OriginalPath){
        ...
        if (stringpath.matches("^[a-zA-Z]:/+script.*")) {
            //將多個slash進行轉換
    ...
    //對input的table name做正規化
    public static String replaceAtt(String s) {
        if (s.endsWith("_ATT")) {
